...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # # Foot clearance reward (penalize low swing height)
        # # foot_positions_w: (num_envs, num_feet, 3)
        # foot_heights = self.foot_positions_w[:, :, 2]
        # target_height = 0.15  # meters

        # # foot_indices: 1 = swing, 0 = stance
        # rew_feet_clearance = torch.sum(
        #     torch.square(foot_heights - target_height) * self.foot_indices,
        #     dim=1
        # )

        # # Contact force reward (encourage stance contact)

        # # net_forces_w: (num_envs, num_bodies, 3)
        # contact_forces = self._contact_sensor.data.net_forces_w

        # # extract foot forces only
        # foot_forces = contact_forces[:, self._feet_ids_sensor, :]

        # # force magnitude
        # foot_force_mag = torch.norm(foot_forces, dim=-1)

        # # reward force during stance only
        # rew_contact_forces = torch.sum(
        #     foot_force_mag * (1.0 - self.foot_indices),
        #     dim=1
        # )
        ...
        # We want gravity ≈ [0, 0, -1] in body frame
        ...
        rewards = {
            ...
            # "rew_contact_forces": rew_contact_forces * self.cfg.tracking_contacts_shaped_force_reward_scale,
        }
    ...
    def foot_positions_w(self) -> torch.Tensor:
        """Returns the foot positions in world frame."""    
    ...
    def _resolve_xy_velocity_to_arrow(self, xy_velocity: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #     assert len(ids) == 1, f"Contact foot {name} not found"
        #     self._feet_ids_sensor.append(ids[0])
        # self._feet_ids_sensor = torch.tensor(self._feet_ids_sensor, device=self.device)
